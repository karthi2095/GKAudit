...
$group=$objlogin->GetGroupById($super['Group_Id']);
...
fclose($output);
	if(!empty($_REQUEST['super_name']))
	{
		$SupervisorName = str_replace(' ', '_', $_REQUEST['super_name']);
	}
	if(!empty($_REQUEST['Zonal_name']))
	{
		$ZonalName = str_replace(' ', '_', $_REQUEST['Zonal_name']);
	}
header("Content-type: application/octet-stream");
//header("Content-disposition:attachment; filename=".$SupervisorName."-".stripslashes($ZonalName)."-".date(Ymd)."-".$group['Group_Name'].".csv");
header("Content-disposition:attachment; filename=".$SupervisorName."-".stripslashes($ZonalName)."-".date(Ymd)."-".trim($group['Group_Name']).".csv");
